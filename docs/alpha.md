...
```sh
curl -LO https://repos.astrazds.net/astrazds/fjgo/releases/download/v1.1.0/fjgo_v1.1.0_linux_amd64.tar.gz
tar -xzf fjgo_v1.1.0_linux_amd64.tar.gz
install -Dm755 fjgo_v1.1.0_linux_amd64/fjgo ~/.local/bin/fjgo
...
```sh
fjgo -R origin issue create --title "fjgo v1.1 field test" --body "Created during fjgo v1.1 validation." --dry-run --yes
fjgo -R origin issue create --title "fjgo v1.1 field test" --body "Created during fjgo v1.1 validation." --yes
fjgo -R origin issue comment ISSUE_NUMBER --body "v1.1 validation comment." --dry-run --yes
fjgo -R origin issue comment ISSUE_NUMBER --body "v1.1 validation comment." --yes
...
fjgo -R origin pr comment PR_NUMBER --body "v1.1 validation PR note." --dry-run --yes
...
fjgo -R origin repo edit --description "fjgo v1.1 validation preview" --dry-run --yes
fjgo repo create fjgo-v1-1-preview --private --dry-run --yes
```
...
fjgo -R origin release create v0.0.0-field-test --body "v1.1 field-test release preview." --dry-run --yes
